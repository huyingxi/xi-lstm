...
class LSTMTagger(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size, word_embed_weight):
        ...
        self.lstmp = RNNModel(input_size = embedding_dim, hidden_size= hidden_dim, recurrent_size=hidden_dim, num_layers=1,num_classes=106, return_sequences=True, bias=True, grad_clip=10, bidirectional=True)
        self.lstmo = RNNModel_O(input_size = embedding_dim, hidden_size = 2*hidden_dim, recurrent_size=2*hidden_dim, num_layers=1, num_classes=106,return_sequences=True, bias=True, grad_clip=10, bidirectional=False)
...
def run():
    ...
    optimizer = optim.RMSprop(model.parameters(), lr=0.1, alpha=0.99, eps=1e-08, weight_decay=0, momentum=0,
                              centered=False)
